...
// Render (and most cloud hosts) block outbound SMTP ports 25/465/587 on
// free/starter plans, which is why the previous raw-SMTP-over-TLS-socket
// implementation worked locally but silently failed once deployed (the
// socket connection just times out). Brevo's transactional email HTTP API
// runs over plain HTTPS (port 443), which is never blocked, so we send
// emails through that instead.
const BREVO_API_URL = "https://api.brevo.com/v3/smtp/email";
// Note: this is the Brevo "API Key" (SMTP & API > API Keys in the Brevo
// dashboard), NOT the SMTP password used by the old BREVO_SMTP_KEY var.
...
function smtpConfigError() {
    ...
    return missing.length
        ? `Brevo API is not configured. Missing: ${missing.join(", ")}`
        : null;
}
...
async function sendSmtpEmail({ to, subject, html, text }) {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Brevo API error ${response.status}: ${details || "failed to send email"}`);
    }
}
